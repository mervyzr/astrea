...
def evolveSpace(tube, gamma, subgrid, boundary):
    extrapolatedValues = reconstruct.extrapolate(tube, gamma, subgrid, boundary)
    limitedValues = limiters.applyLimiter(extrapolatedValues, subgrid)
    solutions = reconstruct.interpolate(extrapolatedValues, limitedValues, subgrid, boundary)
    return solvers.calculateRiemannFlux(tube, solutions, gamma, subgrid, boundary)


# Evolve the system in time by a standardised workflow
def evolveTime(domain, fluxes, dx, dt, stepper, gamma, subgrid, boundary):
    Lq0 = getL(fluxes, dx)
