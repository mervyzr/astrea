...
def printOutput(instanceTime, seed, cfg, **kwargs):
    ...
    if kwargs:
        ...
        print(f"[{instanceTime} | {_seed}] TEST={_config}, CELLS={_cells}, CFL={_cfl}, SUBGRID={_subgrid}, ITERATE={_timestep} || Elapsed: {_elapsed} ({kwargs['runLength']})")
        pass
    else:
        print(f"[{instanceTime} | {_seed}] TEST={_config}, CELLS={_cells}, CFL={_cfl}, SUBGRID={_subgrid}, ITERATE={_timestep} || {bcolours.WARNING}RUNNING SIMULATION..{bcolours.ENDC}", end='\r')
        pass
