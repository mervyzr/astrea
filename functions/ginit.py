    ...
    if multidimensional:
        ...
        if dimensions > 2:
            ...
            if ('sedov' in config) or match(all, ['mhd', 'blast']) or (match(any, ['supernova', 'tycho']) or config == 'sn'):
                ...
                K, B_phi, GM, L, beta0 = test_specifics['K'], test_specifics['B_phi'], test_specifics['GM'], test_specifics['L'], test_specifics['beta0']
                r = np.sqrt(x0**2 + y0**2)

                _P = K * init_cond[rho]**gamma
                cA2 = 2 * (K/beta0) * (init_cond[rho] * r0**2)**(gamma-1)
                cs2 = np.sqrt(gamma * init_cond[pressure]/init_cond[rho])
                torus_phi = -GM/r0 + L**2/(2*r0**2) + (2*cs2 + gamma*cA2)/(2*(gamma-1))

                computational_grid[...,rho] = (
                    mfuncs.divide(
                        np.maximum(0, torus_phi + GM/r0 - L**2/(2*r**2)),
                        K * (gamma/(gamma-1)) * (1 + (r**(2*(gamma-1)))/beta0)
                    )
                )**(1/(gamma-1))
                computational_grid[...,vx] = -np.sqrt(GM * L**2) * (y/r**1.5)
                computational_grid[...,vy] = np.sqrt(GM * L**2) * (x/r**1.5)
                computational_grid[...,pressure] = _P
                computational_grid[...,Bx] = -B_phi * y/r
                computational_grid[...,By] = B_phi * x/r
                ...
                smoothing = lambda q: q * np.exp(-.5 * (R/sigma)**2)

                computational_grid[...,rho] += smoothing(init_cond[rho])
                computational_grid[...,pressure] += smoothing(.25 * ambient[rho] * (omega*sigma)**2)
            ...
            if ('sedov' in config) or match(all, ['mhd', 'blast']) or (match(any, ['supernova', 'tycho']) or config == 'sn'):
                ...
                computational_grid[...,vx] = -np.sqrt(GM * L**2) * (y/r**2)
                computational_grid[...,vy] = np.sqrt(GM * L**2) * (x/r**2)
                computational_grid[...,pressure] = _P
                computational_grid[...,Bx] = -B_phi * y/r
                computational_grid[...,By] = B_phi * x/r
                ...
                if 'blob' in config:
                    ...
                    core_r = shock_pos
                    ring_r = core_r + test_specifics['ring_width']
                    phi = (ring_r - r)/(ring_r - core_r)

                    ring = np.where((r <= (ring_r-x_centre)) & (r <= (ring_r-y_centre)))
        ...
        if ('sedov' in config) or match(all, ['mhd', 'blast']) or (match(any, ['supernova', 'tycho']) or config == 'sn') or config.startswith('sq'):
            if match(any, ['supernova', 'tycho']) or config == 'sn':
                ...
                if 'sedov' in config:
                    computational_grid[...,pressure] = ambient[pressure] + mfuncs.smoothing_kernel(init_cond[pressure], x, d=dimensions, mu=x_centre, sigma=r0)
