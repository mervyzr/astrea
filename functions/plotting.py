...
def plot_quantities(hdf5, sim_variables, save_path):
    ...
    ref_N = 0
    for datetime, grp in hdf5.items():
        ref_datetime = datetime if grp.attrs['subgrid'] == "ppm" else datetime
        ref_N = grp.attrs['cells'] if grp.attrs['subgrid'] == "ppm" else ref_N
    ref_timings = plot_timings_for_each_grp[ref_datetime]
    ...
    for snap_index in range(snapshots):
        ...
        for datetime in datetimes:
            ...
            config = simulation.attrs['config']
            dimension = simulation.attrs['dimension']
            subgrid = simulation.attrs['subgrid']
            solver = simulation.attrs['solver']
            timestep = simulation.attrs['timestep']
            ...
            for idx, (_i,_j) in enumerate(plot_['indexes']):
                ...
                if len(hdf5) != 1:
                    if dimension < 2:
                        ...
                        ax[_i,_j].plot(x, y, label=subgrid.upper())
                        #ax[_i,_j].plot(x, y, label=solver.upper())
                        #ax[_i,_j].plot(x, y, label=timestep.upper())
                ...
                if _i == 0:
                    ax[_i,_j].xaxis.tick_top()
                    ax[_i,_j].xaxis.set_label_position('top')
            ...
            # Add Sod or Sedov analytical solution, using the highest resolution and timing
                ...
                try:
                    if "sod" in config:
                        ...
                        plot_label = r"Sod$_\mathrm{theo}$"
                    ...
                    for idx, (_i,_j) in enumerate(plot_['indexes']):
                        ax[_i,_j].plot(x, y_theo[idx], color=plot_['colours']['theo'], linewidth=1, linestyle="--", label=plot_label)

            #sim_variables = sim_variables._replace(cells=8192)
            #shu_osher_analytical = make_data(options, analytical_config, sim_variables)
            #for idx, (_i,_j) in enumerate(plot_['indexes']):
            #        ax[_i,_j].plot(analytical_grid, shu_osher_analytical[idx], linewidth=1, color=plot_['colours']['theo'], linestyle="--", label=r'$N = 8192$')
        ...
        plt.subplots_adjust(wspace=0.2, hspace=0.065)
...
def plot_total_variation(hdf5, sim_variables, save_path):
    ...
    for _, datetime in enumerate(datetimes):
        ...
        for idx, (_i,_j) in enumerate(plot_['indexes']):
            ax[_i,_j].plot(x, y_data[idx], color=plot_['colours']['1d'][_], label=subgrid.upper())
    ...
    plt.subplots_adjust(wspace=0.3, hspace=0.065)
    #fig.set_size_inches(9, 5)
    #plt.tight_layout()
    ...
    for snap_index in range(snapshots):
        ...
        for datetime in datetimes:
            ...
            for idx, (_i,_j) in enumerate(plot_['indexes']):
                ...
                if len(hdf5) != 1:
                    if dimension < 2:
                        #ax[_i,_j].plot(x, y, label=f"N = {cells}")
                        ax[_i,_j].plot(x, y, label=subgrid.upper())
                        #ax[_i,_j].plot(x, y, label=solver.upper())
                        #ax[_i,_j].plot(x, y, label=timestep.upper())
