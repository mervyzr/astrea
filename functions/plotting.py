...
def plotQuantities(f, simVariables, savepath):
    config, dim, subgrid, timestep, scheme, precision, snapshots = simVariables.config, simVariables.dim, simVariables.subgrid, simVariables.timestep, simVariables.scheme, simVariables.precision, int(simVariables.snapshots)
    ...
    for time_index in range(snapshots):
            ...
            for _i, _j in plotIndexes:
                if len(f) != 1:
                    if dim >= 2:
                        ...
                        plt.suptitle(rf"Primitive variables $\vec{{w}}$ against cell position $x$ at $t \approx {round(timings[max(nList)][time_index],3)}$", fontsize=24)
                else:
                    if dim >= 2:
                        ...
                        plt.suptitle(rf"Primitive variables $\vec{{w}}$ against cell position $x$ at $t \approx {round(timings[max(nList)][time_index],3)}$ ($N = {N}$)", fontsize=24)

        # Add analytical solutions only for 1D
        if dim >= 2:
            plt.suptitle(rf"Primitive variables $\vec{{w}}$ against cell positions $x$ & $y$ at $t \approx {round(timings[max(nList)][time_index],3)}$ ($N = {N}$)", fontsize=24)
        ...
        # Add analytical solutions only for 1D
            ...
            # Add Sod analytical solution, using the highest resolution and timing
                ...
                tube, _t = f[str(max(nList))][str(timings[max(nList)][time_index])], timings[max(nList)][time_index]
        ...
        plt.savefig(f"{savepath}/wPlot_{config}_{subgrid}_{timestep}_{scheme}_{round(timings[max(nList)][time_index],3)}.png", dpi=330)
