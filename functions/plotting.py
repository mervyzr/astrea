...
def plot_quantities(f, sim_variables, save_path):
    ...
    timings = {}
    for dt in dt_list:
        _timings = np.fromiter(f[dt].keys(), dtype=precision)
        _timings.sort()
        timings[dt] = [timing[-1] for timing in np.array_split(_timings, abs(snapshots))]
    time_ref = timings[dt_list[-1]]
    #for N in n_list:
    ...
    for time_index in range(snapshots):
        ...
        for dt in dt_list:
            ...
            time_key = str(timings[dt][time_index])
            grid = simulation[time_key]
            ...
            for _i, _j in PLOT_INDEXES:
                ...
                if len(f) != 1:
                    if dimension < 2:
                        ...
                        plt.suptitle(rf"Primitive variables $\vec{{w}}$ against cell position $x$ at $t \approx {round(time_ref[time_index],3)}$", fontsize=24)
                else:
                    if dimension == 2:
                        graph = ax[_i,_j].imshow(y, interpolation="nearest", cmap=TWOD_COLOURS[_i][_j], origin="lower")
                        divider = make_axes_locatable(ax[_i,_j])
                        cax = divider.append_axes('right', size='5%', pad=0.05)
                        fig.colorbar(graph, cax=cax, orientation='vertical')
                        plt.suptitle(rf"Primitive variables $\vec{{w}}$ against cell indices $x$ & $y$ at $t \approx {round(time_ref[time_index],3)}$ ($N = {N}^{dimension}$)", fontsize=24)
                        fig.text(0.5, 0.04, r"Cell index $x$", fontsize=18, ha='center')
                        fig.text(0.04, 0.4, r"Cell index $y$", fontsize=18, ha='center', rotation='vertical')
                    else:
                        if BEAUTIFY:
                            ...
                            ax[_i,_j].plot(x, y, linewidth=2, color=COLOURS[_i][_j])
                        plt.suptitle(rf"Primitive variables $\vec{{w}}$ against cell position $x$ at $t \approx {round(time_ref[time_index],3)}$ ($N = {N}$)", fontsize=24)
        ...
        if dimension < 2:
            ...
            # Add Sod analytical solution, using the highest resolution and timing
                ...
                grid, _t = f[dt_list[-1]][str(time_ref[time_index])], time_ref[time_index]
        ...
        plt.savefig(f"{save_path}/varPlot_{dimension}D_{config}_{subgrid}_{timestep}_{scheme}_{'%.3f' % round(time_ref[time_index],3)}.png", dpi=330)
