...
def extrapolant_limit(grid, sim_variables, axis, *args, **kwargs):
    ...
    if sim_variables.ppm_author.casefold().startswith(("mccorquodale", "m", "mc")):
        ...
        d2w_C = gutils.slice_(padded_grid, axis, end=-2) - 2*grid + gutils.slice_(padded_grid, axis, start=2)

        # Approximation to the third derivative [McCorquodale & Colella, 2011, eq. 23]
        d3w = gutils.slice_(np.diff(gutils.add_boundary(d2w_C, sim_variables, axis=axis), axis=axis), axis, start=1)

        # Check for cell extrema in cells [McCorquodale & Colella, 2011, eq. 24-25]
        cell_extrema = (dw_minus*dw_plus <= 0) | ((grid-gutils.slice_(padded_grid_2, axis, end=-4)) * (gutils.slice_(padded_grid_2, axis, start=4)-grid) <= 0)

        # If there are extrema in the cells
        if cell_extrema.any():
            d2w_Cw = gutils.add_boundary(d2w_C, sim_variables, axis=axis)
            d2w_lim = np.zeros_like(grid)

            # Get the curvatures that have the same signs
            non_monotonic = (np.sign(gutils.slice_(d2w_Cw, axis, end=-2)) == np.sign(gutils.slice_(d2w_Cw, axis, start=2))) \
                & (np.sign(d2w) == np.sign(d2w_C)) \
                & (np.sign(d2w_C) == np.sign(gutils.slice_(d2w_Cw, axis, end=-2))) \
                & (np.sign(d2w_C) == np.sign(gutils.slice_(d2w_Cw, axis, start=2))) \
                & (np.sign(d2w) == np.sign(gutils.slice_(d2w_Cw, axis, end=-2))) \
                & (np.sign(d2w) == np.sign(gutils.slice_(d2w_Cw, axis, start=2)))
            ...
            limited_curvature = np.sign(d2w_C) * np.minimum(
                ...
                np.minimum(C*np.abs(gutils.slice_(d2w_Cw, axis, start=2)), C*np.abs(gutils.slice_(d2w_Cw, axis, end=-2))) \
                )
            ...
            rho_sensitive = np.abs(d2w) > 1e-12 * np.maximum(
                np.abs(grid), 
                np.maximum(
                    np.maximum(np.abs(gutils.slice_(padded_grid, axis, end=-2)), np.abs(gutils.slice_(padded_grid, axis, start=2))), 
                    np.maximum(np.abs(gutils.slice_(padded_grid_2, axis, end=-4)), np.abs(gutils.slice_(padded_grid_2, axis, start=4)))
                    )
                )

            # Update the limited estimates based on the condition [McCorquodale & Colella, 2011, eq. 27]
            phi = mfuncs.divide(d2w_lim, d2w)
            rho_limiter[rho_sensitive] = phi[rho_sensitive]

            # Apply additional limiters
            d3w_w2 = gutils.add_boundary(d3w, sim_variables, stencil=2, axis=axis)
            d3w_w = gutils.slice_(d3w_w2, axis, *[1,-1])
            d3w_min = np.minimum(
                np.minimum(gutils.slice_(d3w_w, axis, end=-2), d3w), \
                np.minimum(gutils.slice_(d3w_w2, axis, end=-4), gutils.slice_(d3w_w2, axis, start=4)) \
                )
            d3w_max = np.maximum(
                np.maximum(gutils.slice_(d3w_w, axis, end=-2), d3w), \
                np.maximum(gutils.slice_(d3w_w2, axis, end=-4), gutils.slice_(d3w_w2, axis, start=4)) \
                )
        ...
        if sim_variables.ppm_author.casefold().startswith(("peterson", "p", "ph", "x")):
            extrapolant_extrema = (gutils.slice_(padded_grid, axis, end=-2)-grid)*(grid-gutils.slice_(padded_grid, axis, start=2)) <= 0
        else:
            ...
            d_wF_minmod_L, d_wF_minmod_R = left_of_centre - gutils.slice_(padded_interface_2, axis, end=-4), gutils.slice_(padded_interface_2, axis, start=4) - right_of_centre
            d_wS_minmod_L, d_wS_minmod_R = grid - gutils.slice_(padded_grid, axis, end=-2), gutils.slice_(padded_grid, axis, start=2) - grid
        ...
        if cell_extrema.any() or extrapolant_extrema.any():
            ...
            D2w = 6 * (left_of_centre - 2*grid + right_of_centre)
            D2w_L = gutils.slice_(padded_grid_2, axis, end=-4) - 2*gutils.slice_(padded_grid, axis, end=-2) + grid
            D2w_C = gutils.slice_(padded_grid, axis, end=-2) - 2*grid + gutils.slice_(padded_grid, axis, start=2)
            D2w_R = grid - 2*gutils.slice_(padded_grid, axis, start=2) + gutils.slice_(padded_grid_2, axis, start=4)
