...
from functions import generic, ginit, plotting
...
def core_run(grid, sim_variables, t=0., chkpt_idx=1):
    ...
    while t <= sim_variables.t_end:
        ...
        if sim_variables.record_all_steps or sim_variables.live_plot or plot_snapshot or create_chkpt_file:
            # Transform grid for visualisation (in primitive variables)
            grid_snapshot = convert("conservative", grid, sim_variables)

            # Save each instance of the system at time t, if record_all_steps
            if sim_variables.record_all_steps:
            ...
            # Update or save snapshot; IO will handle the live-plot/save-snapshot exclusion condition
            if sim_variables.live_plot:
        ...
        if t == sim_variables.t_end:
            break  # Hard exact stop for the simulation; prevents overshooting the end time by one timestep
        else:
...
def run(seed=SEED, save_dir=SAVE_DIR) -> None:
    ...
    def graceful_exit(sig, frame):
        ...
        print(f"{generic.BColours.WARNING}Simulation ended by user (SIGINT); exiting gracefully...{generic.BColours.ENDC}")
        sys.exit(0)
    ...
    config_variables = handler.allocate(seed, current_dir, db_path, save_path, file_name)
    ...
    # Generate the grid size; if using --test, then a list of increasing power-2 grid sizes is automatically generated
    ...
    try:
        # Initiate the HDF5 database to store data, if record_all_steps
        if sim_variables.record_all_steps:
            handler.write_hdf5(file_name, sim_variables, script_start=script_start)
        ...
        for grid_size in grid_sizes:
            ...
            if sim_variables.record_all_steps:
                handler.append_grp(file_name, sim_variables)
            ...
            if sim_variables.chkpt_file:
                ...
                core_run(grid, sim_variables, t=t, chkpt_idx=chkpt_idx)
            else:
