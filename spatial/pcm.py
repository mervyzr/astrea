...
def reconstruct1d(grid, sim_variables, axis):
    return grid, grid

def reconstruct2d(grid, sim_variables, axis):
    return grid, grid

def reconstruct3d(grid, sim_variables, axis):
    return grid, grid
...
def run(grid, sim_variables):
    guards, multidimensional, magnetic, ds = sim_variables.guards, sim_variables.multidimensional, sim_variables.magnetic, sim_variables.ds[axis]

    Riemann_solver = solvers.get_Riemann_solver(sim_variables)

    if grid.ndim - 1 == 3:
        nx, ny, nz, n = grid.shape

        for i in nb.prange(guards, nx-guards):
            for j in nb.prange(guards, ny-guards):
                for k in nb.prange(guards, nz-guards):

                    rho_p = grid[i+1, j, k, n]
                    rho_m = grid[i  , j, k, n]
