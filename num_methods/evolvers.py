...
def evolve_time(grid, fluxes, dt, sim_variables):
    ...
    def refine_grid(_grid, _sim_variables=sim_variables):
        if _sim_variables.magnetic_2d:
    ...
    if sim_variables.timestep.startswith("ssprk"):
        ...
        if order == 4:
            if register == 10:
                ...
                k5 = refine_grid(3/5*grid + 6/15*k + 1/15*dt*compute_L(fluxes, sim_variables))
                fluxes = evolve_space(k5, sim_variables)

                # Computation of i-th registers (i = 6,7,8,9)
                _k = np.copy(k5)
                for _ in range(4):
                    _k += refine_grid(1/6*dt*compute_L(fluxes, sim_variables))
                    fluxes = evolve_space(_k, sim_variables)

                # Computation of 10th register
                return refine_grid(-11/35*grid + 5/7*k5 + 3/5*_k + 1/10*dt*compute_L(fluxes, sim_variables))
                ...
                k1 = refine_grid(grid + .39175222657189*dt*L_zero)
                fluxes1 = evolve_space(k1, sim_variables)

                # Computation of 2nd register
                k2 = refine_grid(.444370493651235*grid + .555629506348765*k1 + .368410593050371*dt*compute_L(fluxes1, sim_variables))
                fluxes2 = evolve_space(k2, sim_variables)

                # Computation of 3rd register
                k3 = refine_grid(.620101851488403*grid + .379898148511597*k2 + .251891774271694*dt*compute_L(fluxes2, sim_variables))
                fluxes3 = evolve_space(k3, sim_variables)

                # Computation of 4th register
                k4 = refine_grid(.178079954393132*grid + .821920045606868*k3 + .544974750228521*dt*compute_L(fluxes3, sim_variables))
                fluxes4 = evolve_space(k4, sim_variables)

                # Computation of 5th register
                return refine_grid(.517231671970585*k2 + .096059710526147*k3 + .06369246866629*dt*compute_L(fluxes3, sim_variables) + .386708617503269*k4 + .226007483236906*dt*compute_L(fluxes4, sim_variables))

        elif order == 3:
            if register == 5:
                # Evolve system by SSP-RK (5,3) method (3rd-order); effective SSP coeff = 0.53 [Spiteri & Ruuth, 2002; Gottlieb et al., 2008]
                # Computation of 1st register
                k1 = refine_grid(grid + .3772689151171*dt*L_zero)
                fluxes1 = evolve_space(k1, sim_variables)

                # Computation of 2nd register
                k2 = refine_grid(k1 + .3772689151171*dt*compute_L(fluxes1, sim_variables))
                fluxes2 = evolve_space(k2, sim_variables)

                # Computation of 3rd register
                k3 = refine_grid(.56656131914033*grid + .43343868085967*k2 + .16352294089771*dt*compute_L(fluxes2, sim_variables))
                fluxes3 = evolve_space(k3, sim_variables)

                # Computation of 4th register
                k4 = refine_grid(.09299483444413*grid + .0000209036962*k1 + .90698426185967*k3 + .00071997378654*dt*L_zero + .34217696850008*dt*compute_L(fluxes3, sim_variables))
                fluxes4 = evolve_space(k4, sim_variables)

                # Computation of 5th register
                return refine_grid(.0073613226092*grid + .20127980325145*k1 + .00182955389682*k2 + .78952932024253*k4 + (dt * (.0027771981946*L_zero + .00001567934613*compute_L(fluxes1, sim_variables) + .29786487010104*compute_L(fluxes4, sim_variables))))
                ...
                # Computation of 2nd register
                k2 = refine_grid(k1 + .5*dt*compute_L(fluxes1, sim_variables))
                fluxes2 = evolve_space(k2, sim_variables)

                # Computation of 3rd register
                k3 = refine_grid(1/6 * (4*grid + 2*k2 + dt*compute_L(fluxes2, sim_variables)))
                fluxes3 = evolve_space(k3, sim_variables)

                # Computation of 4th register
                return refine_grid(k3 + .5*dt*compute_L(fluxes3, sim_variables))

            else:
                # Evolve system by SSP-RK (3,3) method (3rd-order); effective SSP coeff = 0.333 [Shu & Osher, 1988; Gottlieb et al., 2008]
                # Computation of 1st register
                k1 = refine_grid(grid + dt*L_zero)
                fluxes1 = evolve_space(k1, sim_variables)

                # Computation of 2nd register
                k2 = refine_grid(.25 * (3*grid + k1 + dt*compute_L(fluxes1, sim_variables)))
                fluxes2 = evolve_space(k2, sim_variables)

                # Computation of the 3rd register
                return refine_grid(1/3 * (grid + 2*k2 + 2*dt*compute_L(fluxes2, sim_variables)))
        ...
        # Computation of 2nd register
        k2 = refine_grid(grid + .5*dt*compute_L(fluxes1, sim_variables))
        fluxes2 = evolve_space(k2, sim_variables)

        # Computation of 3rd register
        k3 = refine_grid(grid + dt*compute_L(fluxes2, sim_variables))
        fluxes3 = evolve_space(k3, sim_variables)

        # Computation of the final update
        return refine_grid(grid + 1/6 * (dt * (L_zero + 2*compute_L(fluxes1, sim_variables) + 2*compute_L(fluxes2, sim_variables) + compute_L(fluxes3, sim_variables))))

    else:
        # Evolve system by a full timestep (1st-order)
        return refine_grid(grid + dt*L_zero)