...
def core_run(grp: h5py, _sim_variables: namedtuple, *args, **kwargs):
    ...
    while t <= _sim_variables.t_end:
        ...
        if t == _sim_variables.t_end:
            ...
            Riemann_fluxes = evolvers.evolve_space(grid, _sim_variables)

            # Compute the maximum eigenvalues for determining the full time step
            dt = _sim_variables.cfl * _sim_variables.dx/Riemann_fluxes.eigmax
            ...
            grid = evolvers.evolve_time(grid, Riemann_fluxes, dt, _sim_variables)
